...
const FarmerRegistration = () => {
  ...
  useEffect(() => {
    if (window.ethereum) {
      ...
      const contractInstance = new web3Instance.eth.Contract(contractABI.abi, supplyChainAddress);
  ...
  const handleRegisterFarmer = async () => {
    ...
    try {
      ...
      console.error(err);
      setError('Error registering farmer. Ensure the contract is deployed and connected.');
    }
  };
  ...
  return (
    <div className='flex mt-30  justify-center flex-col items-center relative'>
      ...
      <NavLink to="/roles" className='uppercase bg-black text-white px-4 py-2 rounded-2xl absolute top-0 left-0 mx-30 hover:cursor-pointer hidden lg:flex justify-center items-center gap-2'>
      <FaArrowLeft className='text-sm' />
        <p>back</p>
      </NavLink>

      <h2 className='text-3xl font-semibold'>Register as a Farmer</h2>
      <div className='flex flex-col items-center mt-5 gap-4 p-4'>
      
      {account && <p><strong>Connected Account:</strong> {account}</p>}
      <input className='bg-gray-200 px-4 h-10 rounded-xl w-64' type='text' placeholder='Name' value={name} onChange={(e) => setName(e.target.value)} />
      <input className='bg-gray-200 px-4 h-10 rounded-xl w-64' type='text' placeholder='Location' value={location} onChange={(e) => setLocation(e.target.value)} />
      <input className='bg-gray-200 px-4 h-10 rounded-xl w-64' type='text' placeholder='Specialization' value={specialization} onChange={(e) => setSpecialization(e.target.value)} />
      <button className='bg-black text-white px-4 text-xl rounded-xl mt-3 py-2 hover:cursor-pointer' onClick={handleRegisterFarmer}>Register as Farmer</button>
